...
export default function UsertChat() {
    ...
    if (selectedUser && userId) {
      const getChatHistory = async () => {
        try {
          const q = query(
            collection(db, "chats"),
            where("expertId", "==", selectedUser.id),
            where("userId", "==", userId),
            orderBy("timestamp"),
            limit(100)
          );
  ...
  const sendMessage = async () => {
    try {
      if (selectedUser && userId && messageText.trim() !== "") {
        ...
        console.error(
          "Cannot send message: selected user, expert ID, or message text is undefined or empty"
        );
      }
    } catch (error) {
